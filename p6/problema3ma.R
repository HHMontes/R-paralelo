...
for (i in 1:length(n))
{
  ...
  {
    probabilidades<-c(1-pi[j]-pv,pi[j],pv)
    tipos<-c("S","I","R")
    agentes<-data.frame(x = runif(n[i], 0, l), y = runif(n[i], 0, l),
                        dx = runif(n[i], -v, v), dy = runif(n[i], -v, v),
                        estado = sample(tipos,n[i],replace = T,probabilidades))
    ...
    m_estados<-matrix(agentes$estado,ncol=tmax+2,nrow = n[i])
    ...
    for (tiempo in 0:(tmax-1)) {
      ...
      agentes$estado<-parSapply(cluster,1:n[i],function(i){
        a<-estados_u[i]
